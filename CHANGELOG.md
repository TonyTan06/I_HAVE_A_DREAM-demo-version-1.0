...
- 新增 `ShadowSystem`，统一管理影子记录点、累计水平移动、生成、重力落地、生命与时间移除，以及记录点和影子状态条绘制。
...
- 近战兵的目标检测范围设为自身水平距离 `150px`，检测到玩家或影子后会面向目标；蓝色眼睛、近战攻击框和粉色刀刃特效同步朝向。无目标时仍按原规则持续挥刀。
...
- Scene 新增统一的 `makeCharacterHitbox()`，玩家、影子、近战兵和远程兵提交给弹道系统的碰撞箱不再分别重复计算；共用尺寸常量更名为 `CHARACTER_WIDTH` 和 `CHARACTER_HEIGHT`。
